...
const DoctorSidebar = ({ onClose }) => {
  ...
  return (
    <aside className="h-full w-64 bg-white border-r border-[#EFE2E8]flex flex-col">
      {/* ============================================ */}
      {/* 🎨 BRAND BLOCK */}
      {/* ============================================ */}
      <div className="px-5 py-5 border-b border-[#EFE2E8]flex items-center justify-between">
        ...
        {onClose && (
          <button
            type="button"
            onClick={onClose}
            className="lg:hidden p-1.5 rounded-lg text-gray-400 hover:bg-[#FBEAF1]hover:text-[#374151] transition-colors"
                ...
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors ${
                      isActive
                        ? "bg-gradient-to-r from-[#E27BA3]to-purple-600 text-white shadow-[0_2px_8px_rgba(79,70,229,0.25)]"
                        : "text-[#6B7280] hover:bg-[#FBEAF1]hover:text-[#1F2937]"
                    }`
                  }
                >
      ...
      {/* 👤 USER CARD (bottom) */}
      {/* ============================================ */}
      <div className="border-t border-[#EFE2E8]p-3">
        <div className="flex items-center gap-3 px-2 py-2 rounded-xl">
          {photoUrl ? (
            <img
              src={photoUrl}
              alt={doctor?.fullName || "Doctor"}
              className="w-10 h-10 rounded-full object-cover border border-[#F0D9E2] flex-shrink-0"
            />
          ) : (
